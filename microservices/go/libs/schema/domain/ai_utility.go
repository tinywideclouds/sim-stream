...
func (af *ActionFill) UnmarshalYAML(unmarshal func(interface{}) error) error {
	// 1. Try backwards compatibility: Is it just a flat float? (e.g., energy: 100.0)
	var flatAmount float64
	if err := unmarshal(&flatAmount); err == nil {
		af.Amount = flatAmount
		af.Curve = "linear" // Default to linear for old configs
		return nil
	}

	// 2. If it's not a float, parse it as the full struct
...
// MeterTemplate defines an internal biological or psychological need for V3 actors.
...
// ActionTemplate defines a dynamic choice a V3 actor can make.
type ActionTemplate struct {
	ActionID  string   `yaml:"action_id"`
	ActorTags []string `yaml:"actor_tags"`
	DeviceID  string   `yaml:"device_id"`

	// THE UPGRADE: Satisfies now uses the ActionFill struct instead of a raw float.
	Satisfies map[string]ActionFill `yaml:"satisfies"`

	Costs    map[string]float64 `yaml:"costs"` // We leave costs as flat linear hits for now
