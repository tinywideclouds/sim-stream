...
func TestNegotiator_ResolveEventTime(t *testing.T) {
	negotiator := orchestrator.NewNegotiator()
	...
	tests := []struct {
		name       string
		lead       orchestrator.Participant
		dependents []orchestrator.Participant
		expected   time.Time
	}{
		{
			name: "Perfect Alignment (No Friction Needed)",
			lead: orchestrator.Participant{ActorID: "parent_1", TargetTime: baseTime, Weight: 1.0},
			dependents: []orchestrator.Participant{
		...
		},
		{
			...
			lead: orchestrator.Participant{ActorID: "parent_1", TargetTime: baseTime, Weight: 1.0, PatienceLimit: 10 * time.Minute},
			dependents: []orchestrator.Participant{
