...
func TestNewSimulationState(t *testing.T) {
	...
	// 3. Check Actor Initialization (including the new Bluesky Macro-Phases)
	...
	// NEW: Verify the Macro-Phase initialized correctly to Sleep
	if parent.CurrentPhase != domain.PhaseTypeSleep {
		t.Errorf("Expected parent_1 to start in Macro-Phase Sleep, got %v", parent.CurrentPhase)
	}
