...
func TestProcessPhysics_JouleHeating(t *testing.T) {
	blueprint := &domain.NodeArchetype{
		...
		Devices: []domain.DeviceTemplate{
			{
				DeviceID: "boiler_1",
				Taxonomy: domain.DeviceTaxonomy{Category: domain.DeviceCategory(6)}, // Water Heating
	...
	_ = core.ProcessPhysics(state, tickDuration, grid, participantCounts)
...
func TestProcessPhysics_ColdWaterMixing(t *testing.T) {
	blueprint := &domain.NodeArchetype{
		...
		Devices: []domain.DeviceTemplate{
			{
				DeviceID:          "shower_1",
				Taxonomy:          domain.DeviceTaxonomy{Category: domain.DeviceCategoryWetAppliance}, // Properly mapped Enum!
	...
	result := core.ProcessPhysics(state, tickDuration, grid, participantCounts)
